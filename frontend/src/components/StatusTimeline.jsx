...
const STEPS = [
  { key: 'Registered', label: 'Registered', icon: <Circle size={16} className="text-muted" /> },
  { key: 'Under Review', label: 'Under Review', icon: <Clock size={16} className="text-amber" /> },
  { key: 'In Progress', label: 'In Progress', icon: <Wrench size={16} className="text-burg" /> },
    ...
    <div className="space-y-2">
      <div className="relative">
        ...
        <div className="absolute top-5 left-0 right-0 h-0.5 bg-border mx-10">
          <div
            className="h-full bg-burg transition-all duration-700"
              ...
              <div key={step.key} className="flex flex-col items-center gap-2 flex-1">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-lg border-2 transition-all duration-500 ${
                    isDone
                      ? 'border-burg bg-burg-bg'
                      : 'border-border bg-white'
                  } ${isCurrent ? 'ring-4 ring-burg/20' : ''}`}
                >
                  {step.icon}
                </div>
